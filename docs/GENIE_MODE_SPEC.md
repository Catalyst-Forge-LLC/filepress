...
| Q3 Version store in git | **Gitignore** `.filepress-genie/` (local scratch; bake files are what you commit) |
...
filepress is intentionally simple: **git-native Markdown**, a shared Essay layout, and a Zen Garden `theme.css`. Import already bootstraps a strong first look from source + inspiration + optional Ollama.
...
**Genie Mode** is that loop: a **dev-only** design cockpit that keeps filepress simple in production while making authoring dramatically more powerful locally.
...
| `theme.css` + `static/` + `filepress.config.ts` as the baked artifacts | Versioned experiments under `.filepress-genie/` |
...
- **No production / preview Genie.** Toolbar, routes, and APIs must be absent from `pnpm filepress preview` and adapter-static output.
- **No visitor comments** (permanent filepress non-goal).
...
On Genie open / first LLM action, filepress should:
...
> Ollama is running, but a tuned model will feel snappier for Genie. Try [Finetuna](https://github.com/Catalyst-Forge-LLC/finetuna) (`pnpm start` → create e.g. `gemma4-ctx32k`), then set `FILEPRESS_OLLAMA_MODEL` to that name.
...
- Finetuna is **recommended companion tooling**, not a hard dependency of filepress.
- filepress must not vendor or auto-run Finetuna; link + short why is enough.
- After Finetuna creates a model, authors point filepress at it via `FILEPRESS_OLLAMA_MODEL` / Genie model picker (M2+).
...
1. Author runs `pnpm filepress dev` on a content site.
...
3. `pnpm filepress build` uses those files—**no Genie code in the artifact**.
...
| --- | --- | --- | --- |
| `pnpm filepress dev` | Yes (default on when DEV) | Yes (Vite middleware / local-only) | Writable |
| `pnpm filepress preview` | No | No | Read-only serve of `build/` |
| `pnpm filepress build` | No (not bundled) | No | Reads active working-tree theme/config/static |
...
| Key | Surface | v1 |
| --- | --- | --- |
| `lede` | `filepress.config.ts` | yes |
| `tagline` | `filepress.config.ts` | yes |
| `logo` | `filepress.config.ts` (path) | yes |
...
**Git:** `.filepress-genie/` is **gitignored**. Authors commit baked `theme.css`, `static/`, and `filepress.config.ts`.

### 6.4 Working tree vs snapshot

| Layer | Role |
| --- | --- |
| `.filepress-genie/versions/*` | Local experiments |
| Site root `theme.css`, `static/images/*`, `filepress.config.ts` | **Active** look Vite + `build` consume |
...
git: commit theme.css / static / config when happy (.filepress-genie/ ignored)
...
| Arbitrary file write | Allowlist: `theme.css`, `filepress.config.ts`, `static/images/`, `.filepress-genie/` |
...
- Dev-only floating Genie stub + `GET /__filepress/genie/health`.
