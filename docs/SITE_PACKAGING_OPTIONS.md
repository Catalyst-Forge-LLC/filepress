# filepress — Site packaging options
...
| --- | --- | --- |
| Identity | `filepress.config.ts` | Yes |
...
There is no clean “import routes from `node_modules`” first-class API. Approaches below either accept duplication, generate it, or move the Kit app into filepress.
...
1. **Minimal site surface:** a new site should be roughly `filepress.config.ts` + `posts/` + optional `static/` (+ dependency / one-line scripts).
...
5. **Pinable engine:** sites can pin a filepress version (git SHA / tag / workspace) so core bumps are deliberate (D4/D10).
...
sites/<name>/           # full SvelteKit app
  ...
  vite.config.ts
  package.json          # depends on @filepress/core
```
...
**Shape:** Same as A, but sites treat `src/routes` as **generated** (like lockfile / `.svelte-kit`). Scaffold or `filepress sync` overwrites them from a template in core. Site repo may gitignore routes, or commit them as “vendored” output.
...
```bash
pnpm filepress dev  --site example-site
pnpm filepress build --site example-site
...
- CLI (or root scripts) sets `FILEPRESS_SITE_ROOT` / `FILEPRESS_CONFIG` to `sites/<name>`, points `createContent({ contentDir })` at that site’s `posts/`, aliases `$site-config` to that site’s `filepress.config.ts`, and merges or copies `static/` into the build.
- Vite root stays `packages/app`; site content is outside the app via absolute paths / env (already close to today’s `FILEPRESS_CONTENT_DIR` seam).
...
| Reuses existing core DI (`createContent`, `defineFilepressConfig`) | Need a small CLI/`--site` convention and tests for path resolution |
...
### Option D — filepress as an app/CLI dependency (Astro / VitePress-like)

**Shape of a site (ideal end state, including external repos)**

```
my-blog/
  package.json          # "filepress": "github:…/filepress#v0.x" or workspace
  filepress.config.ts
  posts/
  static/
```
...
- `@filepress/core` (engine)
- The SvelteKit app (`packages/app`)
- A `filepress` bin that runs Vite/Kit with `cwd` = the site root

**Deploy**

| Setting | Value |
| --- | --- |
| Build command | `pnpm install && pnpm build` (i.e. `filepress build`) |
...
| Benefits | Tradeoffs |
| --- | --- |
| True “link filepress + config + posts” | Must solve: resolve site root, load TS config, map `static/`, HMR for out-of-package `posts/` |
| External site repos stay tiny | Packaging complexity (bin, peer deps on Svelte/Vite, Windows paths) |
| Engine upgrades = bump dependency | Debugging “my site” means stepping into `node_modules/filepress` |
...
**Relationship to C:** C is the in-monorepo form of the same architecture; D is the packaging/distribution form. Implement C first, then expose the same app via a `filepress` binary for D.
...
1. **Promote routes into `packages/app`** — one SvelteKit application owned by filepress.
2. **Demote `sites/*` to content packages** — only `filepress.config.ts`, `posts/`, `static/`, and maybe a one-line README. Remove per-site `src/routes`, `vite.config.ts`, and site-local Kit deps.
3. **Add a thin CLI / root scripts** — `filepress dev|build --site <name>` (monorepo) that injects site root into the app (config alias, content dir, static assets, output dir).
4. **Keep `@filepress/core` as the library** — app depends on core; sites do not need to import core directly.
...
Duplication is already complete between two sites. A third site multiplies dead weight. The user’s ideal (“link filepress and configure the bare minimum”) is Option D; A cannot get there without pretending `src/routes` doesn’t exist.
...
| What `filepress.config.ts` must have | Correct canonical `url` (feeds/sitemap/SEO). Not CF credentials. |
...
2. Wire site injection: `FILEPRESS_SITE_ROOT` → config path, `posts/`, `static/`, output dir.
...
- Keep site identity validation loud (`defineFilepressConfig`) regardless of packaging.
