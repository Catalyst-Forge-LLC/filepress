...
| Version rail + activate | Gitignored `.filepress-genie/`; each row shows the applied look and the original prompt (click to copy); bake into `theme.css` / `static/` |
...
Hand-edited `theme.css` remains fully supported. Genie never ships in `preview` or production builds. Spec: [GENIE_MODE_SPEC.md](/docs/genie). Ollama optional for the LLM path; [Finetuna](https://github.com/Catalyst-Forge-LLC/finetuna) recommended for a tuned local model.
